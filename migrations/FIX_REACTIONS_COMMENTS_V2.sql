-- FIX V2: Reactions and Comments not working
-- Run this in Supabase SQL Editor
-- Handles existing tables with different column names
...
-- =============================================================================
-- STEP 3: Fix leaderboard_entries table - add missing columns
-- =============================================================================
-- Add total_points column if it doesn't exist (might be named 'points')
DO $$
BEGIN
    -- Check if total_points column exists
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'leaderboard_entries' AND column_name = 'total_points'
    ) THEN
        -- Check if points column exists and rename it
        IF EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_name = 'leaderboard_entries' AND column_name = 'points'
        ) THEN
            ALTER TABLE leaderboard_entries RENAME COLUMN points TO total_points;
            RAISE NOTICE 'Renamed points column to total_points';
        ELSE
            -- Add total_points column
            ALTER TABLE leaderboard_entries ADD COLUMN total_points INTEGER NOT NULL DEFAULT 0;
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns
        WHERE table_name = 'leaderboard_entries' AND column_name = 'rank'
    ) THEN
        ALTER TABLE leaderboard_entries ADD COLUMN rank INTEGER;
...
DO $$
BEGIN
    RAISE NOTICE '✅ Fix V2 applied successfully!';
