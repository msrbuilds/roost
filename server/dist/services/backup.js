...
async function compressFile(inputPath) {
    ...
    await (0, promises_1.pipeline)((0, fs_1.createReadStream)(inputPath), (0, zlib_1.createGzip)({ level: 9 }), (0, fs_2.createWriteStream)(outputPath));
...
async function performBackup() {
    ...
    try {
        ...
        const fileStats = (0, fs_1.statSync)(compressedPath);
        const fileSizeMB = (fileStats.size / (1024 * 1024)).toFixed(2);
        // Step 4: Clean up local temp file
        (0, fs_1.unlinkSync)(compressedPath);
        tempFilepath = null;
 ...
 */
async function getBackupDownloadUrl(filename) {
    const s3Key = `${BACKUP_PREFIX}${filename}`;
