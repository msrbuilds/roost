...
function generateFileKey(folder, fileName) {
 ...
 */
exports.uploadRouter.post('/presign', async (req, res) => {
    // Authenticate
    ...
    try {
        ...
        const presignedUrl = await (0, s3_request_presigner_1.getSignedUrl)(s3Client, command, { expiresIn: 300 }); // 5 min
        res.json({
            presignedUrl,
            key,
 ...
 */
exports.uploadRouter.post('/delete', async (req, res) => {
    const userId = await authenticateUser(req);
    if (!userId) {
        return res.status(401).json({ error: 'Authentication required' });
    }
    ...
    try {
        ...
        if (!isAdmin) {
            ...
            const { data: asset } = await supabase_js_1.supabaseAdmin
                .from('assets')
                .select('uploaded_by')
