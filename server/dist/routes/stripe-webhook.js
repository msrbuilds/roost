...
async function logWebhook(eventType, stripeEventId, payload, ipAddress) {
...
async function updateMembershipType(userId, type) {
    await supabase_js_1.supabaseAdmin
        .from('profiles')
        .update({
...
async function handleSubscriptionChange(subscription) {
    ...
    // Invalidate caches
    await (0, cache_js_1.invalidateWebhookRelatedCaches)(userId);
}
...
exports.stripeWebhookRouter.post('/', async (req, res) => {
    ...
    let event;
    try {
        event = (0, stripe_js_1.constructWebhookEvent)(req.body, signature);
    }
    catch (err) {
    ...
    // Idempotency check
    const { data: existingLog } = await supabase_js_1.supabaseAdmin
        .from('stripe_webhook_logs')
        .select('id')
    ...
    // Log the event
    const logId = await logWebhook(event.type, event.id, event.data.object, ipAddress);
    try {
        switch (event.type) {
...
// Health check
exports.stripeWebhookRouter.get('/health', (_req, res) => {
