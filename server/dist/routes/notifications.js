...
router.post('/process-emails', async (req, res) => {
    ...
    try {
        ...
        const { data: pendingNotifications, error: fetchError } = await supabase_js_1.supabaseAdmin
        ...
        for (const notification of pendingNotifications) {
            results.processed++;
            try {
                const emailSent = await email_js_1.emailService.sendNotificationEmail({
                ...
                if (emailSent) {
                    ...
                    await supabase_js_1.supabaseAdmin.rpc('mark_notification_email_sent', {
...
router.get('/status', async (req, res) => {
    ...
    try {
        ...
        const { count: pendingCount } = await supabase_js_1.supabaseAdmin
            .from('notifications')
            .select('*', { count: 'exact', head: true })
            .eq('email_pending', true);
        // Get sent emails in last 24 hours
        const yesterday = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString();
        const { count: sentCount } = await supabase_js_1.supabaseAdmin
            .from('notifications')
            .select('*', { count: 'exact', head: true })
