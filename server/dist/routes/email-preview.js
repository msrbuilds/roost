...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: #667eea; padding: 40px 20px; text-align: center; border-radius: 12px 12px 0 0;">
    <h1 style="color: white; margin: 0; font-size: 28px;">Welcome to Commune!</h1>
  </div>
  ...
  <div style="background: #ffffff; padding: 40px 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
    ...
    <p style="margin-bottom: 0;">Best,<br><strong>The Commune Team</strong></p>
  </div>

  <div style="text-align: center; padding: 20px; color: #9ca3af; font-size: 12px;">
    <p style="margin: 0;">&copy; ${new Date().getFullYear()} Commune. All rights reserved.</p>
...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <div style="background: #ffffff; padding: 40px 30px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
    ...
    <p>We received a request to reset the password for your Commune account associated with <strong>${safeEmail}</strong>.</p>
 ...
 */
router.get('/subscription-cancelled', (req, res) => {
    if (!isDev) {
        return res.status(404).json({ error: 'Not found' });
    }
    const safeName = escapeHtml(sampleData.name);
 ...
 */
router.get('/subscription-reactivated', (req, res) => {
    if (!isDev) {
        return res.status(404).json({ error: 'Not found' });
    }
    const safeName = escapeHtml(sampleData.name);
