...
const two_factor_1 = require("../services/two-factor");
const router = (0, express_1.Router)();
// Setup 2FA schema
const setup2FASchema = zod_1.z.object({
...
const verifyBackupCodeSchema = zod_1.z.object({
    code: zod_1.z.string().min(1, 'Backup code is required'),
});
// Check 2FA required for login schema
const check2FARequiredSchema = zod_1.z.object({
 ...
 */
router.post('/check-required', async (req, res) => {
    try {
        ...
        if (requires2FA) {
            ...
            await supabase_1.supabaseAdmin.auth.admin.signOut(signInData.session.access_token);
            return res.json({
                requires2FA: true,
...
router.post('/verify-login', async (req, res) => {
    try {
        ...
        // Verify the 2FA code
        const isValid = (0, two_factor_1.verifyTOTP)(profile.two_factor_secret, code);
        if (!isValid) {
 ...
 */
router.post('/verify-login-backup', async (req, res) => {
    try {
        const schema = zod_1.z.object({
            email: zod_1.z.string().email(),
            password: zod_1.z.string().min(1),
        ...
        // Verify backup code
        const codeHash = (0, two_factor_1.hashBackupCode)(code);
        const { data: backupCode, error: findError } = await supabase_1.supabaseAdmin
...
router.post('/setup', async (req, res) => {
    try {
        ...
        // Generate new secret
        const secret = (0, two_factor_1.generateSecret)();
        const otpauthUri = (0, two_factor_1.generateOtpauthUri)(secret, user.email || '', 'Let\'s Vibe It');
...
router.post('/verify', async (req, res) => {
    try {
        ...
        const backupCodes = (0, two_factor_1.generateBackupCodes)();
        // Hash and store backup codes
        const backupCodeRecords = backupCodes.map((c) => ({
            user_id: user.id,
            code_hash: (0, two_factor_1.hashBackupCode)(c),
        }));
        ...
        const { error: backupError } = await supabase_1.supabaseAdmin
            .from('two_factor_backup_codes')
        ...
        // Enable 2FA
        const { error: enableError } = await supabase_1.supabaseAdmin
            .from('profiles')
            .update({
...
router.post('/disable', async (req, res) => {
    try {
        ...
        const { error: disableError } = await supabase_1.supabaseAdmin
            .from('profiles')
            .update({
            two_factor_enabled: false,
